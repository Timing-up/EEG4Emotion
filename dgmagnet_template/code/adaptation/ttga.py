...
def ttga_adapt(
    ...
    # Save initial value for anchored regularization: penalize drift from init,
    # not drift from zero — aligns with mean-init strategy.
    embed_init = target_embed.data.clone().to(target_samples.device)
    ...
    for step in range(steps):
        ...
        frob = ((target_embed - embed_init) ** 2).sum()  # anchored to init, not zero
